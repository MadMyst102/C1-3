      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'INIT_DATA') {
            this.initialData = {
              cashiers: data.cashiers.map((c: any) => ({
                ...c,
                deliveries: c.deliveries.map((d: any) => ({
                  ...d,
              ...
              reports: data.reports.map((r: any) => ({
                ...r,
                reports: r.reports.map((cr: any) => ({
                  ...cr,
                  deliveries: cr.deliveries.map((d: any) => ({
                    ...d,
                    timestamp: new Date(d.timestamp)
                  }))
                }))
...
const wsManager = new DBWebSocketManager('ws://localhost:3000');
